...
var e = fs.readFileSync(workerData.data,{
    encoding: "utf8"
})
...
/* Getting result */
var result = []
parentPort.postMessage(`analyzing from ${workerData.range.join(" to ")} ...`)
// console.log("analyzing ...")

for (var i=workerData.range[0]; i<=workerData.range[1]; i++){
    // console.log(e)
    // let dTemp = [data].flat()
    let flag = true
    let oNow = {
    ...
    if(linkage.length > 0) {
        ...
        parentPort.postMessage(rObj["ticket_trace/contact"])
        result.push(rObj)
    }
    else {
        let rObj = {
...
try {
    /* 
        Parsing json to csv
    */
   parentPort.postMessage(`parsing from ${workerData.range.join(" to ")} ...`)
    const parser = new Parser(opts)
    const csv = parser.parse(result)
    fs.writeFileSync(`./result_3_${workerData.range.join("_")}.csv`, csv, function(err){
